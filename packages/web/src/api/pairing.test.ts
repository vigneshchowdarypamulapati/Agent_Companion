...
describe('claimPairingCode', () => {
  ...
  it('resolves when the relay accepts the code', async () => {
    ...
    await expect(claimPairingCode('tok-1', '123456')).resolves.toBeUndefined();
  });
  ...
  it('posts the code with a Bearer authorization header', async () => {
    ...
    await claimPairingCode('tok-1', '123456');
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it('throws UnauthorizedError on 401', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: false, status: 401, json: async () => ({}) })));
    await expect(claimPairingCode('bad-token', '123456')).rejects.toBeInstanceOf(UnauthorizedError);
  });
  ...
  it("throws the relay's error message when the response body carries one", async () => {
    ...
    await expect(claimPairingCode('tok-1', '123456')).rejects.toThrow('Pairing code already claimed');
  });
  ...
  it('throws a generic HTTP message when the body cannot be parsed', async () => {
    ...
    await expect(claimPairingCode('tok-1', '123456')).rejects.toThrow('HTTP 500');
  });
});
